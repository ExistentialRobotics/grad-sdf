...
BindAabbTemplate(const py::module &m, const char *py_class_name) {
    ...
    auto py_aabb = py::class_<Cls>(m, py_class_name)
                       .def(
                           py::init<typename Cls::Point, typename Cls::Scalar>(),
                           py::arg("center"),
                           py::arg("half_size"))
                       .def(
                           py::init<typename Cls::Point, typename Cls::Point>(),
                           py::arg("min"),
                           py::arg("max"));
    if (Dim == 1) {
        py::enum_<typename Cls::CornerType>(
            py_aabb,
            "CornerType",
            py::arithmetic(),
            "Type of corners.")
    ...
    py_aabb.def_readonly("center", &Cls::center)
        .def_readonly("half_sizes", &Cls::half_sizes)
        .def_property_readonly("min", py::overload_cast<>(&Cls::min, py::const_))
        .def_property_readonly("max", py::overload_cast<>(&Cls::max, py::const_))
        .def(
            "__contains__",
            [](const Cls &aabb, const Eigen::Vector<Scalar, Dim> &point) {
        ...
        .def(
            "padding",
            py::overload_cast<const typename Cls::Point &>(&Cls::Padding, py::const_),
            py::arg("padding"))
        .def(
            "padding",
            py::overload_cast<typename Cls::Scalar>(&Cls::Padding, py::const_),
            py::arg("padding"))
