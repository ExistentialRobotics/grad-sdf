...
namespace libmorton {
    ...
    morton
    m3D_e_sLUT(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_sLUT_ET(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_LUT(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_LUT_ET(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_magicbits(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_for(coord x, coord y, coord z);
    template<typename morton, typename coord>
    morton
    m3D_e_for_ET(coord x, coord y, coord z);

    // AVAILABLE METHODS FOR DECODING
    template<typename morton, typename coord>
    void
    m3D_d_sLUT(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_sLUT_ET(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_LUT(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_LUT_ET(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_magicbits(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_for(morton m, coord& x, coord& y, coord& z);
    template<typename morton, typename coord>
    void
    m3D_d_for_ET(morton m, coord& x, coord& y, coord& z);

    // ENCODE 3D Morton code : Pre-Shifted LookUpTable (sLUT)
    template<typename morton, typename coord>
    morton
    m3D_e_sLUT(const coord x, const coord y, const coord z) {
        morton answer = 0;
        for (unsigned int i = sizeof(coord); i > 0; --i) {
            unsigned int shift = (i - 1) * 8;
            answer = answer << 24 | (Morton3D_encode_z_256[(z >> shift) & EIGHTBITMASK] |
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_LUT(const coord x, const coord y, const coord z) {
        morton answer = 0;
        for (unsigned int i = sizeof(coord); i > 0; --i) {
            unsigned int shift = (i - 1) * 8;
            answer = answer << 24 |
    ...
    template<typename morton, typename coord>
    morton
    compute3D_ET_LUT_encode(const coord c, const coord* LUT) {
        unsigned long maxbit = 0;
        if (findFirstSetBit<coord>(c, &maxbit) == 0) { return 0; }
        morton answer = 0;
        for (int i = static_cast<int>(std::ceil(static_cast<float>(maxbit + 1) / 8.0f)); i >= 0;
             --i) {
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_sLUT_ET(const coord x, const coord y, const coord z) {
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_LUT_ET(const coord x, const coord y, const coord z) {
        morton answer_x = compute3D_ET_LUT_encode<morton, coord>(x, Morton3D_encode_x_256);
    ...
    static uint_fast32_t magicbit3D_masks32_encode[6] = {
        ...
        0x0300f00f,
        0x30c30c3,
        0x9249249};  // we add a 0 on position 1 in this array to use same code for 32-bit and
                     // 64-bit cases
    ...
    template<typename morton, typename coord>
    static morton
    morton3D_SplitBy3bits(const coord a) {
        ...
        morton x = static_cast<morton>(a) & masks[0];
        if (sizeof(morton) == 8) {
            x = (x | static_cast<uint_fast64_t>(x) << 32) & masks[1];
        }  // for 64-bit case
        x = (x | x << 16) & masks[2];
        x = (x | x << 8) & masks[3];
        x = (x | x << 4) & masks[4];
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_magicbits(const coord x, const coord y, const coord z) {
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_for(const coord x, const coord y, const coord z) {
        morton answer = 0;
        unsigned int checkbits = (sizeof(morton) * 8) / 3;
        for (unsigned int i = 0; i < checkbits; ++i) {
            morton mshifted = static_cast<morton>(1)
                              << i;  // Here we need to cast 0x1 to 64bits, otherwise there is a bug
                                     // when morton code is larger than 32 bits
    ...
    template<typename morton, typename coord>
    morton
    m3D_e_for_ET(const coord x, const coord y, const coord z) {
        morton answer = 0;
        ...
        checkbits = std::min(
            static_cast<unsigned long>(checkbits),
            std::max(z_max, std::max(x_max, y_max)) + 1ul);
        for (unsigned int i = 0; i < checkbits; ++i) {
            morton m_shifted = static_cast<morton>(1)
                               << i;  // Here we need to cast 0x1 to 64bits, otherwise there is a
                                      // bug when morton code is larger than 32 bits
    ...
    template<typename morton, typename coord>
    coord
    morton3D_DecodeCoord_LUT256(
        const morton m,
        const uint_fast8_t* LUT,
        const unsigned int startshift) {
        morton a = 0;
    ...
    template<typename morton, typename coord>
    void
    m3D_d_sLUT(const morton m, coord& x, coord& y, coord& z) {
    ...
    template<typename morton, typename coord>
    void
    m3D_d_LUT(const morton m, coord& x, coord& y, coord& z) {
        x = morton3D_DecodeCoord_LUT256<morton, coord>(m, Morton3D_decode_x_512, 0);
    ...
    template<typename morton, typename coord>
    void
    m3D_d_sLUT_ET(const morton m, coord& x, coord& y, coord& z) {
        x = 0;
        y = 0;
        z = 0;
        ...
        while (firstbit_location > i) {
            ...
            x |= static_cast<coord>(Morton3D_decode_x_512[m_shifted]) << shiftback;
            y |= static_cast<coord>(Morton3D_decode_y_512[m_shifted]) << shiftback;
            z |= static_cast<coord>(Morton3D_decode_z_512[m_shifted]) << shiftback;
            shiftback += 3;
            i += 9;
        }
    }
    ...
    template<typename morton, typename coord>
    void
    m3D_d_LUT_ET(const morton m, coord& x, coord& y, coord& z) {
        x = 0;
        y = 0;
        z = 0;
        unsigned long firstbit_location = 0;
        ...
        while (i < firstbit_location) {
            x = x | static_cast<coord>(Morton3D_decode_x_512[(m >> i) & NINEBITMASK]) << shiftback;
            y = y | static_cast<coord>(Morton3D_decode_x_512[(m >> (i + 1)) & NINEBITMASK])
                        << shiftback;
            z = z | static_cast<coord>(Morton3D_decode_x_512[(m >> (i + 2)) & NINEBITMASK])
                        << shiftback;
            i += 9;
            shiftback += 3;
        }
    ...
    static uint_fast32_t magicbit3D_masks32_decode[6] = {
        ...
        0x0300f00f,
        0x30c30c3,
        0x9249249};  // we add a 0 on position 0 in this array to use same code for 32-bit and
                     // 64-bit cases
    ...
    template<typename morton, typename coord>
    static coord
    morton3D_GetThirdBits(const morton m) {
    ...
    template<typename morton, typename coord>
    void
    m3D_d_magicbits(const morton m, coord& x, coord& y, coord& z) {
    ...
    template<typename morton, typename coord>
    void
    m3D_d_for(const morton m, coord& x, coord& y, coord& z) {
        x = 0;
        y = 0;
        z = 0;
        unsigned int checkbits = (sizeof(morton) * 8) / 3;
    ...
    template<typename morton, typename coord>
    void
    m3D_d_for_ET(const morton m, coord& x, coord& y, coord& z) {
        x = 0;
        y = 0;
        z = 0;
        unsigned long firstbit_location = 0;
        ...
        unsigned int checkbits = static_cast<unsigned int>(std::min(
            static_cast<float>(defaultbits),
            static_cast<float>(firstbit_location) / 3.0f));
        for (unsigned int i = 0; i <= checkbits; ++i) {
            morton selector = 1;
            unsigned int shift_selector = 3 * i;
            unsigned int shiftback = 2 * i;
